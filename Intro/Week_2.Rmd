...
ggplot(data = nycflights, aes(x = dep_delay)) +
  geom_histogram(color='white',fill='red',binwidth = 20)+theme_bw()
```
...
ggplot(data = nycflights, aes(x = dep_delay)) +
  geom_histogram(color='white',fill='red',binwidth = 15)+theme_bw()
ggplot(data = nycflights, aes(x = dep_delay)) +
  geom_histogram(color='white',fill='red',binwidth = 150)+theme_bw()
```
...
ggplot(data = rdu_flights, aes(x = dep_delay)) +
  geom_histogram(color='white',fill='red',binwidth=10)+theme_bw()
```
...
sfo_feb_flights <- nycflights %>%
  filter(dest == "SFO", month == 2) # for and operation you can use comma
```
...
ggplot(data = sfo_feb_flights, aes(x = arr_delay)) +
  geom_histogram(color='white',fill='red',binwidth=10)+theme_bw()
sfo_feb_flights %>%
  summarise(mean_ad = mean(arr_delay), sd_dd = sd(arr_delay), sample_size = n())
```
...
`sfo_feb_flights` data frame, grouped by carrier. Which carrier is the has the hightest
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(median_ad = median(arr_delay), iqr = IQR(arr_delay))
```
...
ggplot(nycflights, aes(x = factor(month), y = dep_delay)) +
  geom_boxplot(color='red')+theme_bw()
```
...
ggplot(data=nycflights,aes(x=avg_speed,y=distance))+geom_point(color='red',alpha=0.4)+theme_bw()
```
...
nycflights %>% filter(dep_type == 'delayed', arr_type == 'on time') # 1767 # 7%
