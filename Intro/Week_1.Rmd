...
arbuthnot <- arbuthnot %>%
  mutate(proportion_boys = boys / sum(arbuthnot$boys))

arbuthnot <- arbuthnot %>%
  mutate(proportion_girls = girls / sum(arbuthnot$girls))

ggplot(data = arbuthnot, aes(x = year, y = proportion_boys)) + geom_line()+ geom_point()+theme_bw()
```
...
ggplot(data=present, aes(x=year,y=prop_boys))+geom_point()+geom_line()+theme_bw()
```
...
ggplot(data=present, aes(x=year,y=prop_boy_girl))+geom_line()+geom_point()+theme_bw()
```
...
#  mutate(total = ?) %>%
#  arrange(desc(total))
present <- present %>% mutate(total = boys + girls) %>% arrange(desc(total))
```
