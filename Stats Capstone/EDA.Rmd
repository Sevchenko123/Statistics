...
# Misc Feature, Pool QC, Alley have most Na's
```
...
# Overall Condition and Overall Quality could take finite values. 
```
...
library(ggplot2)
ggplot(ames_train,aes(x=Neighborhood,y=price))+geom_boxplot(color='black',fill='firebrick1')+theme_bw()+labs(y="Prices",x="Neighbourhood")

ames_train %>% filter(Neighborhood == "StoneBr") %>% summarise(max_S = max(price),min_S=min(price),sd_S=sd(price),avg_S=mean(price),med_S=median(price))

ames_train %>% filter(Neighborhood == "Timber") %>% summarise(max_T = max(price),min_T=min(price),sd_T=sd(price),avg_T=mean(price),med_T=median(price))

ames_train %>% filter(Neighborhood == "Veenker") %>% summarise(max_V = max(price),min_V=min(price),sd_V=sd(price),avg_V=mean(price),med_V=median(price))

ames_train %>% filter(Neighborhood == "NridgHt") %>% summarise(max_N = max(price),min_N=min(price),sd_N=sd(price),avg_N=mean(price),med_N=median(price))

# StoneBr has highest standard deviation in terms of the price.
```
...
```{r fig.width=14, fig.height=7,Q4}
...
g = ggpairs(ames_train,columns = c(3,7,53,20,22), lower = list(continuous = my_fn))
g

# Overall Quality looks highly correlated with price.
```
...
ggplot(ames_train,aes(x=log(area),y=log(price)))+geom_point(pch=19,color='blue',alpha=0.5)+geom_smooth(method=lm,color='red',se=F)+theme_bw()+labs(x="Area",y="Price")
...
# When both price and area are log transformed the correlation is the greatest.
```
...
ames_train %>% summarise(mean_price = mean(price),median_price = median(price))
ames_train %>% filter(Total.Bsmt.SF == 0) %>% summarise(n())
summary(ames_train$Street)

# 21 houses don't have a basement is true.
```
...
# p value is < .00001 so reject null hypothesis.
```
...
#bayes_inference(y=Bedroom.AbvGr,data=Bed,statistic = "mean",type="ht",null=3,alternative = "twosided")
```
...
log_area <- log(ames_train$area)
summary(log_area)
IQR(log_area)
summary(log_price)
IQR(log_price)

# index 645,855,918 are outliers 
# Overall Quality is less than 3
```
...
# 3 Neighborhoods consist only single family home
```
...
ggplot(ames_train,aes(x=log(area),y=Bedroom.AbvGr))+geom_point(pch=19,color='blue',alpha=0.5)+geom_smooth(method=lm,color='red',se=F)+theme_bw()+labs(x="Area",y="Bedrooms")

cor(log_area,ames_train$Bedroom.AbvGr)
```
...
r1 <- ames_train %>% filter(BsmtFin.Type.1=="Unf"| BsmtFin.Type.2=="Unf") %>% dplyr::select(Bsmt.Unf.SF)
mean(r1$Bsmt.Unf.SF)
```
