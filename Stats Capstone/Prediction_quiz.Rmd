...
predicted <- exp(predict(model.AIC,ames_test))
...
rmse.full 
# rmse for model.full on ames_test 33293.55
```
...
# Extract Predictions
predict.full <- exp(predict(model.AIC, ames_test))
...
# rmse for model.full on ames_test is 33293.55
# rmse for model.full on ames_train is 22706.55
# rmse for model.AIC on ames_train is 22823.41
# rmse for model.AIC on ames_test is 33136.8
...
model.BIC <- stepAIC(model.full,direction="backward",k=log(1000))
...
pred.test.HPM <- predict(model.bas, newdata = ames_test, estimator="BPM")
pred.HPM.rmse <- sqrt(mean((exp(pred.test.HPM$fit) - ames_test$price)^2))
pred.HPM.rmse
