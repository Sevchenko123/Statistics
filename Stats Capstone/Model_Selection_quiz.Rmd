...
model <- lm(log(price)~log(area)+log(Lot.Area)+Bedroom.AbvGr+Overall.Qual+Land.Slope,data=ames_train) # build a model
#step1 <- stepAIC(model, direction="forward",k=2) # Forward Elimination Method to select the best AIC model.
#step1$anova # display best model based on AIC criterion
#summary(model)

step2 <- stepAIC(model,direction="forward",k=log(1000)) # Forward Elimination method to select best BIC model.
step2$anova # display the best model based on BIC criterion
```
...
model <- lm(log(price)~log(area),data=ames_train)
ames_resid <- cbind(ames_train,resid(model))
colnames(ames_resid)[82] <- "resids"
ames_resid %>% group_by(Neighborhood) %>% summarise(m=mean(resids))%>% arrange(desc(m))
# GrnHill
```
...
model <- lm(log(price)~Overall.Qual+Bsmt.Qual+Garage.Qual,data=ames_train)
length(resid(model))
#64
```
...
model <- lm(log(price)~Overall.Cond+Overall.Qual,data=ames_train)
model_predictions <- predict(model,ames_train)
ames_resid <- cbind(ames_resid,model_predictions)
ames_resid %>% group_by(MS.SubClass)%>%summarise(med=median(price))%>%arrange(desc(med))
#Option B
```
...
#option b because we are using log
```
...
ames_filter <- ames_train %>% filter(Sale.Condition=="Normal")
#83.4
```
...
model <- lm(log(price)~log(area),data=ames_filter)
summary(model)
#option A
