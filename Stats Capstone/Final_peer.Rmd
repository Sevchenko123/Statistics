...
**3. The third plot shows the boxplot of price based on the heating quality and kitchen quality of the house. As expected, the price is high if the quality is relatively good. There are only two houses with poor kitchen and heating quality. **


```{r fig.width=15, fig.height=8 ,creategraphs,warning=FALSE}
...
**I calculated the RMSE of all the three models. The RMSE of the first model is 36310.92 dollars which is the best out of all the three models. The RMSE of second model which is based on AIC criterion is 36417.36 dollars. Finally the Bayesion regression model performs the worst out of all with RMSE of 78388.54.**
...
**The RMSE on the test data is quite more than the train data. This shows model has been overfitted.**
...
**The adjusted R squared of this model has increased from the previous model. This model is performing better than the previous model built on the test data as the RMSE is 61406.27 dollars as compared to RMSE of 67084.22 in the previous model.**
...
**Strengths include that the model is a very good fit as per the adjusted R squared with adj. R squared of almost 0.90. Weakness of the model could be that it seems to have overfitted as the RMSE on the test data is more than RMSE than train data. But evidence shows that the training data fits your model a lot better than the test data.**
...
The model performs better on validation set. The RMSE of model on validation test is 59179.66 dollars which is better than RMSE on test set. It still is not better as compared to training set. Almost 89% of the 95% predictive confidence intervals contain the true price of the house in the validation data set. So the model can deal very well with uncertainties. 
