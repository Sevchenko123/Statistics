...
summary(wage$wage)
ggplot(wage,aes(x=wage,y=..density..))+geom_histogram(color="white",fill="red")+theme_bw()+labs(x="Wage in Dollars",y="Total")+geom_density(color="black")
```
...
g = ggpairs(wage,columns = c(1,2,6), lower = list(continuous = my_fn))
g
```
...
ggplot(data = wage, aes(x = iq, y = wage))+geom_point(pch=19,color='black',alpha=0.5)+geom_smooth(method=lm, fill="red", color="red", se=FALSE)+theme_bw()+labs(x="IQ",y="Wage")
```
...
```{r educ-resid}
# type your code for the Exercise here, and Knit
m_wage_edu = lm(wage ~ iq+educ, data = wage)
m_wage_edu$coefficients
summary(m_wage_edu)$sigma
```
...
# type your code for the Exercise here, and Knit
library(MASS)
stepAIC(m_lwage_full,k= log(935),data=na.omit(wage))
```
