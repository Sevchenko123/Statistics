...
summary(nc$weight)
ggplot(nc,aes(x=weight,y =..density..))+geom_histogram(color="white",fill="red")+theme_bw()+labs(x="Weight in Pounds",y="Total Babies")+geom_density(color="black")
```
...
```{r normal-ci}
# type your code for the Exercise here, and Knit
ggplot(nc,aes(x=habit,y=weight))+geom_boxplot(fill="red",color='black')+theme_bw()
```
...
```{r}
bayes_inference(y = lowbirthweight, data = nc, success = "low", statistic = "proportion", type = "ht", null = 0.075, alternative = "twosided",beta_prior = c(0.3,0.7))
```
...
```{r}
bayes_inference(y = premie, x = habit, success="premie", data = nc, statistic = "proportion", type = "ht", null = 0, alternative = "twosided")
```

```{r}
bayes_inference(y = lowbirthweight, x = habit, success="low", data = nc, statistic = "proportion", type = "ht", null = 0, alternative = "twosided")
```