...
nc_1 <- nc %>% filter(!is.na(habit), !is.na(weight))
ggplot(nc_1,aes(habit,weight))+geom_boxplot(color='black',fill='red')+theme_bw()
```
...
nc %>%
  group_by(mature) %>%
  summarise(range_weight = max(mage))
```
...
```{r pick-your-own}
# type your code for the Exercise here, and Knit
inference(y = gained, x = whitemom, data = nc, statistic = "mean", type = "ci", 
          method = "theoretical",order=c("white","not white"))
```
