...
geom_histogram(color = 'white', fill = 'blue', binwidth = 250)+theme_bw()+labs(y="Number of Homes",x="Area")
```
...
ggplot(data = ci_data, aes(x = ci_bounds, y = ci_id, 
  ...
  geom_vline(xintercept = params$mu, color = "black")+theme_bw() # draw vertical line
```
...
ci_data_99 <- ci_99 %>% mutate(capture_mu = ifelse(lower < params$mu & upper > params$mu, "yes", "no"))

ci_data_99 <- data.frame(ci_id = c(1:50, 1:50),
                      ci_bounds = c(ci_99$lower, ci_99$upper),
                      capture_mu = c(ci_data_99$capture, ci_data_99$capture))
